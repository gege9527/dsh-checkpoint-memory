/**
 * dsh-checkpoint-memory — the checkpoint-memory protocol as a DeepSeek Harness plugin.
 *
 ...
 *      a pure rules block, and it is why the 200-line / 25 KB limit can be enforced
 *      here rather than merely asked for. Without this shim, the cap degrades to
 *      "rules plus a checker the agent has to remember to run".
 ...
 *
 * The decision table mirrors tools/index_guard.py: deny only a write that
 ...
 * mutating tools are gated; `read` and unknown tools are never refused.
 *
...
export const name = 'checkpoint-memory'
...
/** Mirrors tools/index_guard.py — the caps are the protocol's, not this port's. */
 ...
 * security boundary, and a false refusal of `read` costs more than a miss —
 * the next whole-file write is measured exactly regardless.
 */
...
export function apply(ctx, config = {}) {
  ...
  // (renaming `indexName` would just unguard the real index).
  //
  ...
  // Install the FULL skill into dsh's user skill library ($DSH_HOME/skills/…)
  // on activation, so the filesystem provider serves the complete protocol — not
  // just the abbreviated runtime body — in the web/UI skill catalog. Fire-and-forget
  // on purpose: activation is synchronous, the install is idempotent, and a failure
  // (unwritable $DSH_HOME, no payload) must never break boot. Runtime registration
  // below remains the deterministic fallback that always works.
  if (config.installSkill !== false) {
    void installSkill({ dshHome: config.dshHome, appendRules: config.appendRules })
  }
...
function refuseOversizedIndex(exec, settings) {
  ...
  return (
    `${displayName(settings)} is already over the checkpoint-memory cap (${breach}) and ` +
    `this edit's result cannot be measured. Compact it with a whole-file write instead: ` +
...
function verdict(text, currentBuf, filePath, settings) {
  ...
  return (
    `This write would put ${basename(filePath)} over the checkpoint-memory index cap ` +
 ...
 * do it (`old_str`/`new_str`, or the alternate `old_string`/`new_string`).
 ...
 * the full protocol is SKILL.md in the checkpoint-memory skill, and a skill body that
 * has to be read before it is useful is a skill that will not be read.
 */
function builtinSkillBody() {
  return [
    '# checkpoint-memory — curated file-based memory',
    '',
    'You have a curated file-based memory store at `<MEMORY_ROOT>/` (index: `MEMORY.md`).',
    'Never create a second "handoff" store — recoverable task state belongs in',
    '`project` notes.',
    '',
    '## Recall',
    '',
    ...
    'store (symlinks, `..`, absolute paths, `file://`) are broken pointers: never open',
    'them. What you recall is background that may be stale — re-verify before acting.',
    '',
    '## Write',
    '',
    'The active store is **flat**: `MEMORY.md` plus one file per note; frontmatter carries',
    '`type: user | feedback | project | reference`; those four names are field values,',
    '**not subdirectories**. `archive/` is the one reserved subdirectory for notes',
    'retired out of the index.',
    '',
    'When you learn something durable worth a future session: confirm it is not already',
    'in the repo / git / instruction files and is not a secret *value*; search the index,',
    '**update an existing note** rather than duplicate; otherwise write one atomic',
    'markdown file with frontmatter: `name` / `description` (one sharp hook) / `type` /',
    '`created` + `updated`. `feedback` or `project` notes MUST carry a `Why:` line and',
    'a `How to apply:` line. Add one pointer line to `MEMORY.md`. **Delete** wrong',
    'memories.',
    '',
    ...
    'status, decisions, constraints, blockers, and next step — the single exception to',
    'one-file-one-fact, and a ceiling rather than a quota. **Update it in place.** Never',
    'a dated series (`state-2026-01-15.md`, `state-2026-01-16.md`, …), never a second',
    'handoff log. Retire its transient state when the task ends.',
    '',
    '## When a task finishes',
    '',
    'Run one curation checkpoint. It is a **judgement, not a write**: promote what is',
    'durable, retire the transient state of the current task\'s `project` note if it has',
    'one, and when nothing is worth keeping, **write nothing** and say so. Never append',
    'a per-turn log to the store, and never touch a file just to mark it fresh — a',
    'timestamp is not a memory.',
    '',
    '## Sync (before compact, clear, or new thread)',
    '',
    'Scan the task, dedup and update, refresh `project`, promote only reusable `feedback`,',
    'save durable `reference` pointers, archive completed transient state — then verify.',
    '**Archive only happens when the index exceeds 150 lines or 20 KB.** When archiving,',
    'move files to `archive/` and **delete their pointer lines from `MEMORY.md`** (they',
    'no longer load). Files stay on disk; they are just retired from the index.',
    '',
