...
> 本插件基于上游仓库 [tinqiao-oss/engramory](https://github.com/tinqiao-oss/engramory) 的
> `adapters/dsh/plugin` 目录修改、重命名后独立发布。上游原始 npm 包名为 `dsh-engramory`，
> skill/插件 ID 为 `engramory`。
...
1. **身份与元数据**：包名、插件 ID、skill 名称从 `engramory` 改为 `checkpoint-memory`；版本重置为 `0.1.0`；移除了上游特有的 `repository`/`homepage`/`bugs`/`publishConfig` 及 `engramory` 关键字。
2. **自带完整 skill 载荷**：新增 `skill/` 目录与 `install-skill.js`，插件激活时把完整 checkpoint-memory 协议幂等地安装到 `$DSH_HOME/skills/checkpoint-memory/`，并自动把 `rules-snippet.md` 追加到 `$DSH_HOME/AGENTS.md`。
3. **Web 同步按钮**：新增 `client.js`，在 Web Composer 的 `conversation.input.right` 槽位注册「同步记忆」按钮，点击后向当前会话提交 `/checkpoint-memory`，在主会话中同时执行召回与策展写入。
4. **协议文案**：守卫核心算法与上限（200 行 / 25 KB）保持与上游 Python 版 `tools/index_guard.py` 一致；内置 skill body、错误提示、README 等全部更新为 `checkpoint-memory` 规则。
5. **文档整理**：删除独立的英文 README，中文文档直接作为主 `README.md`。

完整修改清单见 [UPSTREAM.md](UPSTREAM.md)。
