...
function mount(config = {}, { withSkills = true } = {}) {
  ...
  const pending = []
  // The skill-to-library install is OFF here by default: that sync targets the real
  // $DSH_HOME, a side effect no hermetic unit test should touch. Install behaviour
  // is covered directly against temp dirs in its own describe block below.
  apply(makeCtx(new Set(inject)), { installSkill: false, ...config })
...
test('plugin metadata is what the loader expects', () => {
  assert.equal(name, 'checkpoint-memory')
...
test('a shrinking whole-file write passes even while the index is still over', () => {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'checkpoint-memory-dsh-'))
...
test('indexPath pins the guard to one file, so a same-named file elsewhere is free', () => {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'checkpoint-memory-pin-'))
  ...
  assert.match(pinned({ name: 'write', arguments: { file_path: real, content: over } }),
    /over the checkpoint-memory index cap/, 'the pinned index must still be guarded')
...
test('a pinned path is compared by identity, not by spelling', () => {
  ...
  assert.match(guard({ name: 'write', arguments: { file_path: real, content: over } }),
    /over the checkpoint-memory index cap/, 'a `..` detour is the same file')
})
...
test('pinning an index that does not exist yet still guards its first write', () => {
  ...
  assert.match(guard({ name: 'write', arguments: { file_path: notYet, content: over } }),
    /over the checkpoint-memory index cap/)
})
...
test('a pinned index under a symlinked ancestor keeps its identity once created', () => {
  ...
  const base = realpathSync(mkdtempSync(join(tmpdir(), 'checkpoint-memory-link-')))
  ...
  assert.match(guard({ name: 'write', arguments: { file_path: pinned, content: over } }),
    /over the checkpoint-memory index cap/, 'the first write to a not-yet-created index')

  writeFileSync(join(realDir, 'MEMORY.md'), '- x\n') // that first write lands

  assert.match(guard({ name: 'write', arguments: { file_path: pinned, content: over } }),
    /over the checkpoint-memory index cap/, 'GUARD WENT SILENT once the index existed')
  assert.match(
    guard({ name: 'write', arguments: { file_path: join(realDir, 'MEMORY.md'), content: over } }),
    /over the checkpoint-memory index cap/, 'the same file by its real path is still the index')
})
...
test('the protocol skill is registered, and carries the parts that matter', () => {
  ...
  assert.equal(skill.name, 'checkpoint-memory')
  ...
  assert.match(flat, /The active store is \*\*flat\*\*/i)
  assert.match(flat, /`archive\/` is the one reserved subdirectory/)
  assert.match(flat, /\*\*at most one\*\* live `project` note/i)
  assert.match(flat, /Update it in place/i)
  assert.match(flat, /never a second handoff log/)
  assert.match(flat, /curation checkpoint/)
  assert.match(flat, /judgement, not a write/)
  assert.match(flat, /per-turn log to the store/i)
...
test('a skill registry that activates after the plugin still gets the skill', () => {
  ...
  assert.equal(late.skills[0].name, 'checkpoint-memory')
