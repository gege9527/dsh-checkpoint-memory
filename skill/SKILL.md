...
6. **更新索引：** 添加指针行。
7. **错误/过时内容：** 证明错误/过时的 → **不删除文件**（删除由用户决定，agent 最高止于归档）。索引未超限时仅把过时内容标记出来告知用户；索引超限、执行 §8 压缩时，才把不再活跃的文件移到 `archive/` 并从索引移除指针行。
...
7. 标记/归档过期笔记和已完成的瞬时状态（未超限仅标记告知用户、不删除文件；超限执行 §8 压缩时才归档；删除由用户决定）
