#!/usr/bin/env python3
"""
checkpoint_memory_doctor — consistency + protocol check for a Checkpoint-Memory memory store.

    python tools/checkpoint_memory_doctor.py <MEMORY_ROOT>              # structure + schema
    python tools/checkpoint_memory_doctor.py <MEMORY_ROOT> --no-schema  # structure only
...
it to health-check a store that isn't (yet) in strict Checkpoint-Memory format, e.g. a
...
are read; the name<->filename check ignores `-`/`_`/case so a name like `a-b`
vs `a_b.md` isn't flagged. The frontmatter grammar is the
...
CHECKPOINT_MEMORY_HARD / CHECKPOINT_MEMORY_HARD_BYTES.
"""
import datetime
import os
import re
...
def _frontmatter(text):
    # Validate + parse Checkpoint-Memory's restricted `key: value` frontmatter between
...
def main(argv):
    ...
    if not os.path.isfile(idx_path):
        print(f"checkpoint-memory-doctor: no index at {idx_path}")
        return 1
    ...
    if not _within(idx_path, root_abs):
        print(f"checkpoint-memory-doctor: index at {idx_path} resolves outside the store root "
    ...
    if isize is not None and isize > parse_cap:
        ...
        print(f"checkpoint-memory-doctor: 1 issue(s) — 1 over-cap")
        print(f"  fix over-cap: compact MEMORY.md — pointer-ify long lines, merge, "
              f"archive cold notes")
        return 1
    iraw = _read_bytes(idx_path)
    if iraw is None:
        print(f"checkpoint-memory-doctor: cannot read index at {idx_path}")
        return 1
    ...
    # the cap math agrees with the hook and checkpoint_memory_check on a non-UTF-8 index.
    nbytes = len(iraw)
    nlines = _lines(itext)
    ...
    for base, p in sorted(notes.items()):
        ...
        for w in re.findall(r"\[\[([^\]]+)\]\]", text):
            # Checkpoint-Memory wikilinks are bare slugs. Tolerate an Obsidian-style display alias
            ...
            if "/" in target or "\\" in target:
                info.append(f"[[{w}]] in {base} isn't a bare slug (checkpoint-memory links are flat "
        ...
        if fm is None and not fm_problems:
            ...
            name = fm.get("name", "")
            if name:
                # tolerate the convention of '-' in names vs '_' in filenames
                # and case; also tolerate a leading type-prefix added to filenames
                # (e.g. name 'audit-methodology' vs file 'feedback_audit_methodology').
                # Only flag a real mismatch (soft).
    ...
    if issues:
        ...
        print(f"checkpoint-memory-doctor: {len(issues)} issue(s) — {summary}")
        for b in order:
            if b in buckets:
    ...
    print(f"checkpoint-memory-doctor: clean — index {nlines} lines / {_kb(nbytes)}, "
